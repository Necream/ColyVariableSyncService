...
string CommandExecutor(string command){
    ...
    if(operation_id==1){ // set
        return "[ERROR]You can't set memory directly, use specific commands like set process or set var.";
    }
    if(operation_id==2){ // get
        return "[ERROR]You can't get memory directly, use specific commands like get process or get var.";
    }
    if(operation_id==3){ // del
        return "[ERROR]You can't delete memory directly, use specific commands like del process or del var.";
    }
    if(operation_id==4){ // sync
        return "[ERROR]You can't sync memory directly, use specific commands like sync process or sync var.";
    }
    if(operation_id==5){ // reg
        return "[ERROR]You can't register memory directly, use specific commands like reg process or reg var.";
    }
