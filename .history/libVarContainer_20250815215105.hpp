...
struct MemoryContainer {
    ...
    json to_json() const {
        json processes_json;
        for (const auto& [proc_name, proc_container] : process_container) {
            processes_json[proc_name] = proc_container.to_json();
        }
        return json{
    ...
    MemoryContainer& operator=(const json& j) {
        ...
        if (j.contains("process_container") && j["process_container"].is_object()) {
            for (const auto& [proc_name, proc_json] : j["process_container"].items()) {
                ProcessContainer pc;
                pc = proc_json;
                process_container[proc_name] = pc;
            }
        }
        Timestamp = j.value("Timestamp", 0);
        return *this;
