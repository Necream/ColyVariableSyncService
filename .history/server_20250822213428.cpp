...
string CommandExecutor(string command, set<shared_ptr<ServerSession>>& clients);
        ...
        socket.async_read_some(asio::buffer(read_buf),
            [this, self](error_code ec, size_t length) {
                if (!ec) {
                    ...
                    // 回显固定消息
                    send_message(CommandExecutor(msg, clients));
...
map<set<shared_ptr<ServerSession>>,string> procid_map; // 用于存储进程ID和会话的映射
void OperationInit(){
...
string CommandExecutor(string command, set<shared_ptr<ServerSession>>& clients) {
    ...
    if(operation_id==51){ // reg process
        ...
        ProcessContainer pc;
        memory_container.process_container[processid] = pc;
        proof_map[processid] = "default_proof"; // 默认凭证
        procid_map[clients] = processid; // 将会话与进程ID
