...
struct ServerSession : enable_shared_from_this<ServerSession>{
    ...
    void close(shared_ptr<ServerSession> client) {
        ...
        if(session_map.find(client)!=session_map.end()) cout<<"Session(ProcessID:"<<session_map[client]<<") closed and resources cleaned up."<<endl;
        if(session_map.find(client)!=session_map.end()) session_map.erase(client); // 从会话映射中删除
    }
};
