...
string CommandExecutor(string command,shared_ptr<ServerSession> client){
    ...
    if(operation_id==53){ // reg subprocess
        ...
        proof_map[subprocessid] = 1; // 初始化子进程凭证
    ...
    if(operation_id==61){ // login process
        ...
        if(proof_map.find(processid) == proof_map.end()){
            cout<<"[ERROR]Process not logged in."<<endl;
            return "[ERROR]Process not logged in.";
        }
        cout<<"Process "<<processid<<" logged in."<<endl;
        return "Process logged in";
    }
    cout<<"[ERROR]Unknown command: "<<command<<endl;
    return "[ERROR]Unknown command"+command;
}
