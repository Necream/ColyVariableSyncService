...
struct ServerSession : enable_shared_from_this<ServerSession>{
    ...
    void close() {
        ...
        socket.close();
        session_map.erase(shared_from_this()); // 从会话映射中删除
        // 删除该会话对应的子进程映射
        if(subprocess_map.find(session_map[this]) != subprocess_map.end()){
            for(const string& subpid:subprocess_map[session_map[this]]){
                proof_map.erase(subpid); // 删除子进程凭证
            }
            subprocess_map.erase(session_map[this]); // 删除子进程映射
        }
    }
};
