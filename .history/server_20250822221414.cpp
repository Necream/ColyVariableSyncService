...
string CommandExecutor(string command,shared_ptr<ServerSession> client){
    ...
    if(operation_id==51){ // reg process
        ...
        proof_map[processid] = session_map[client]; // 初始化进程凭证
    ...
    if(operation_id==61){ // login process
        string processid=command;
        if(proof_map.find(processid) == proof_map.end()){
            cout<<"[ERROR]Process not found, please register first."<<endl;
            return "[ERROR]Process not found, please register first.";
        }
        session_map[client] = proof_map[processid]; // 将会话与进程ID关联
        proof_map.erase(processid); // 清除进程凭证
    }
    if(operation_id==63){ // login subprocess
        ...
        session_map[client] = proof_map[sub]; // 将会话与子进程ID关联
