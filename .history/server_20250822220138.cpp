...
string CommandExecutor(string command,shared_ptr<ServerSession> client){
    ...
    if(operation_id==52){ // reg subprocess
        string processid="";
        for(char c:command){
            if(c==' '){
                break;
        ...
        if(memory_container.process_container[processid].SubProcesses.find(subprocessid) != memory_container.process_container[processid].SubProcesses.end()){
        ...
        ProcessContainer sub_pc;
        memory_container.process_container[processid].SubProcesses[subprocessid] = sub_pc; // 初始化子进程容器
        cout<<"Subprocess "<<subprocessid<<" registered under process "<<processid<<"."<<endl;
